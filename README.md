...
The included fixture has 12 rows for software testing: eight composition/score pairs from Seko’s published examples and four project-authored test cases. They are not predictions generated by Eka; see [third-party notices](THIRD_PARTY_NOTICES.md). Installation downloads Julia dependencies; queries and tests need no network access after installation.
...
This evaluates supplied rankings; it does not train a predictive model or establish
...
the supplied MP terms support preparation of attributed results and covered data
under CC BY 4.0; see the [terms review](docs/mp-terms-evidence.md). The
...
**This pipeline imports existing numeric scores; it does not train a model or generate predictions from unscored compounds.** A rebuilt database uses this package's standard schema, not a byte-for-byte copy of the legacy source.
...
A new CLI invocation starts a new Julia process. It is **not** equivalent to a second function call in an existing process. A small precompilation workload exercises representative queries and CLI parsing against a temporary test database to reduce repeated compilation costs. Initial installation/precompilation is a separate one-time cost and can recur after source/dependency changes.
...
Eka's Julia implementation, Materials Project tooling and recovery benchmark
workflows are developed by **Joshua Corbett**.
...
- **Seko database:** the optional SQLite workflow can query the separately
  ...
  Eka supports its schema and query conventions; it does not bundle their Python
  script or implement their tensor model. A small set of their published example
  rows is retained in software fixtures, as detailed in [third-party notices](THIRD_PARTY_NOTICES.md).
...
Preserve Materials Project source/version information and applicable citations
when using MP data; see the [data/provenance review](docs/mp-data-provenance-review.md).

## Licence

Eka's original code, documentation and original test material are licensed under
the [MIT License](LICENSE), copyright © 2026 Joshua Corbett.

Reused Seko examples and dependencies retain their own notices; see
[third-party notices](THIRD_PARTY_NOTICES.md). MIT does not grant redistribution
rights over Materials Project or Seko datasets, third-party records, or
data-derived artifacts. Their separate status is recorded in the
[publication-permissions register](docs/publication-permissions.md).
Scientific citation requests do not add conditions to MIT.
