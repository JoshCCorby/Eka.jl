...
function universe(groups)
    ...
    Eka.mp_recovery_splits(pairs; seeds=[0], budgets=[1])
...
function system_splits(groups, policy; seeds=0:19, budgets=[20,50,100,200])
    seeds = Eka.recovery_integers(seeds,"seeds";maximum=typemax(Int)-10000)
    budgets = Eka.recovery_integers(budgets,"budgets";minimum=1)
...
function source_groups(source)
    rows=Eka.recovery_group_rows(source.files["audit/compositions.tsv"])
...
function capture_baseline(path,source;synthetic)
    bytes=read(joinpath(path,"config.toml")); config=Eka.recovery_toml(bytes,"sensitivity config")
    pin=Eka.recovery_protocol(LS.PROTOCOL)
    ...
    seeds=Eka.recovery_integers(config["split_seeds"],"baseline seeds")
    budgets=Eka.recovery_integers(config["budgets"],"baseline budgets";minimum=1)
...
function write_rank(path,ranked,heldout)
    ...
    Eka.pu_write_rows(path,keys(first(rows)),rows)
    return rows
end
...
function run_system_holdout(snapshot,audit,baseline_path,output;synthetic=false,preflight_only=false)
    ...
    if preflight_only
        mkdir(target)
        Eka.pu_write_rows(joinpath(target,"populations.tsv"),keys(first(branches).population),[b.population for b in branches])
    ...
    names=sort!(unique([collect(Eka.PU_PRODUCER_FILES);"src/mp_pu.jl";"src/mp_label_sensitivity.jl";
    ...
    try
        ...
        for branch in branches
            ...
            if design=="system"
                Eka.pu_write_rows(joinpath(folder,"selected-systems.tsv"),(:chemical_system,),
                    [(chemical_system=s,) for s in selections(universe(groups),split.seed)])
            else
                for n in Eka.PU_MEMBER_FILES
            ...
            for method in METHODS
                ...
                if method=="similarity"
                    ...
                    Eka.pu_write_rows(joinpath(folder,"candidate-similarity.tsv"),keys(first(detail)),detail)
